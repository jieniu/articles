...
3. 最后，将第 1 步的结果除以 sum，得到 $y_1' = y_1 / sum$，$y_2'=y_2/sum$
...
> 在信息论中，熵 $H=\sum -y\log(y)$ 用来描述系统中蕴含的信息量的期望，其中 $-\log(y)$ 是自信息，表示概率为 y 的事件所对应的信息量，小概率事件的信息量更大，反之亦然。
>
> 而交叉熵描述的是消除系统中的信息，所付出的努力，且当 y' 和 y 相等时，所付出的努力最小。
>
...
一、将输入数据矩阵与参数矩阵做矩阵乘法，输入数据矩阵的行数为样本数，每行中的内容为每条样本的特征，以此达到批量计算的目的，这样乘出来的结果为每条样本在不同模型上的输出，又一个分类对应一个模型，即每个样本对不同分类的初始预测值；
...
这里先用 one-hot 编码将目标向量转化为和预测结果一样的矩阵形式，如预测结果为 `n*q` 的矩阵（n 表示一次预测 n 条样本，q 表示分类数），那么 one-hot 编码会将目标向量转化为 `n*q` 的矩阵；

接着再对预测矩阵和目标矩阵做一个“与操作” `boolean_mask` 就可以把正确分类对应的预测值取出来了；

最后对预测值求 `-log` 后再求和，就是这批样本的 Loss，代码如下：
...
这次我们可以使用准确率（accuracy）来评估模型的效果，准确率意为预测正确的数量的占比。

评估模型时，要先对数据进行预测，再拿预测结果和正确分类做对比，以此统计正确的预测次数。这里使用的 `tf.argmax` 函数，表示在多个预测分类中，取最大的那个值作为预测结果：
...
以上为模型训练、预测和评估所需要的通用方法，接下来我们就可以来训练模型了，本次我们使用的是 fashion_mnist 数据集，其中每个样本为一张 28*28 像素的图片，目标值 label 为这张图片所属的类别，如下所示：

![fation ](https://github.com/jieniu/articles/blob/master/docs/.vuepress/public/fation .jpg?raw=true)
...
把参数弄清楚后，剩下的就是模型迭代了，这一部分和上一篇[线性回归](/AI/linear-scratch.md)的训练代码大致一样，代码如下：
